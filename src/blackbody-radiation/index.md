...
**Try the linear axis.** The *Linear* button redraws the same curves against a plain wavelength axis from 0 to 4 μm. For the Sun this is the picture in the textbooks: a steep rise, a peak, and a long tail, and it is the honest one for judging power by eye, since on a linear axis the area under the curve *is* the power. Now drag toward the cold end. Once the peak passes the middle of the axis, at 2 μm and 1449 K, the radiance axis stops following it, because a curve that is sliding out of the frame is no longer a sensible thing to size the frame by. From there on the curve simply sinks, as the fifth power says it should, and by *Earth's surface* it is gone: its peak is at 10 μm, two and a half axis-lengths off to the right, and what little it emits below 4 μm is a hundred-thousandth of the frame's height. The slider stops short as well, at 724 K, because it can only reach wavelengths the axis has. No linear axis can show sunlight and earthlight together, which is the reason the default here is logarithmic, and it is a fair picture of how far apart the two are.
