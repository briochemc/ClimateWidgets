...
Sunlight comes in mostly as visible light, and the Earth sends the energy back out as infrared, at wavelengths twenty times longer. The air in between is not equally clear to both. Water vapour, carbon dioxide and a few other gases each absorb at their own wavelengths, and where they do, light that would have gone straight through is stopped. This figure shows what gets through, gas by gas, along the same wavelength axis as the [blackbody widget](../blackbody-radiation/): the top panel is the two glows, sunlight and the Earth's, each drawn faintly in full and solidly where it gets through; the middle panel is the fraction the whole atmosphere absorbs; and the rows below are the constituents, one each, in two groups: oxygen, ozone and the scattering by the air itself, which mostly act on the incoming sunlight, and then the greenhouse gases, strongest first, which mostly act on the outgoing glow.

Left alone, the figure starts with the first group alone, an atmosphere with no greenhouse gases, and adds those one at a time: water vapour, carbon dioxide, methane, nitrous oxide. Press anything and the tour stops (reload the page to see it again). Tick a row to include that constituent or leave it out (every combination is a few clicks: untick everything but carbon dioxide, say), and hover or touch the figure to read the absorbed fraction at any wavelength. Under *More controls*, the segmented controls set the amount of water vapour (a dry polar column, the global mean, or the humid tropics) and of carbon dioxide, methane and nitrous oxide (1750, today, or double today's), and the slider moves the Earth's surface temperature; while that section is open each row also shows its amount.
...
**Carbon dioxide's band gets wider, not deeper.** Untick everything but carbon dioxide and switch its amount from 1750 to today to doubled. The centre of its 15 μm band was already completely opaque in 1750; more gas cannot make it more opaque. What changes is the edges: the band widens, at 13 and at 17 μm, and each time it widens it takes another slice of the Earth's curve. That is why "the CO₂ band is saturated" is not the objection it sounds like. The same is true of the 4.3 μm band, which matters less because neither the Sun nor the Earth is bright there.

**A colder or a warmer surface.** The slider under *More controls* moves the Earth's surface across the range the real surface spans, from −90 °C, the Antarctic plateau's record low, to 60 °C, a desert afternoon, and its curve slides along the axis under bands that stay where they are (Wien's law: the peak wavelength is inversely proportional to the absolute temperature). At 15 °C the peak, at 10 μm, sits in the window. Cool the surface to −60 °C, an Antarctic winter night, and the peak moves out to 13.6 μm, onto the carbon dioxide band and the far-infrared water bands: with everything included, the share that escapes falls from 28% at 15 °C to 18%, and to 12% at −90 °C. Warm it to 37 °C and the peak moves to 9.3 μm, deeper into the window, and 30% escapes. It is a small effect on the share, because the window is wide and the curve broad, but it is one part of why a warmer surface cools itself more efficiently through the window, and why a cold one does not.

**Water vapour does most of the absorbing, in bands, and its amount is set by the temperature.** Untick everything but water vapour: most of the infrared picture is already there. Water's bands are wide and numerous, and on an average day there are about ten thousand water molecules in the air for every CO₂ molecule. But water vapour condenses out in days and its amount follows the temperature, so it is not a control on the climate but an amplifier. The dry gases, carbon dioxide first, set the temperature; the water vapour follows and roughly doubles their effect.

**Dry air, mean air, tropical air.** The amount of water in the column varies more than anything else in the figure: about 4 kg per square metre over a polar or subarctic winter, 25 for the globe as a whole, 40 and more in the humid tropics, a factor of ten between the extremes. The water-vapour control offers the three. Sunlight hardly notices: 79%, 73% and 71% of it reaches the ground, because water's near-infrared bands sit on the tail of the solar curve. The outgoing glow does: 36%, 28% and 26% of it escapes straight to space with everything else at today's amounts. Note the asymmetry. Drying the air from the mean to polar opens the far infrared beyond 17 μm and lets a good deal more out; wetting it to tropical closes only the last edges of bands that were already opaque. (In reality the tropical case is more opaque still, because of the water-vapour continuum discussed below, which grows as the square of the humidity.) That dry air lets the ground radiate almost freely is why the winter poles cool so hard, and why the greenhouse effect of a given amount of CO₂ is largest where the air is driest.
...
display(embedSnippets({
  ...
  note: "The figure is a fixed 640&nbsp;px tall; below it are a line of help and the collapsed " +
    "<em>More controls</em> section, which needs about 90&nbsp;px more when opened (more at phone " +
    "width, where its controls wrap). The embed page accepts " +
    ...
    "work too), <code>?h2o=dry</code> (<code>mean</code>, <code>tropical</code>) to open on that water " +
    "column, <code>?gases=h2o,co2</code> to open with only those constituents included, " +
    "<code>?earth=-50</code> to open with the surface at that temperature in °C (−90 to 60), and " +
...
<script type="module">
  ...
  // Options: {amounts: {h2o: "tropical", co2: 278, ch4: "1750", n2o: "doubled"}} opens on those amounts,
  // {gases: ["h2o", "co2"]} with only those constituents included, {earthTemperature: -50}
  // with the surface at that temperature in °C (−90 to 60), {tour: false} without the tour.
